 ...
 * not-found error assembly). Every Kadena crypto primitive is INJECTED through
 ...
 * binds the real `smartDecrypt` / `KadenaWalletBuilder` / `kadenaDecrypt` /
...
/**
 * The seed types a Kadena seed can carry. Mirrors `@stoachain`'s
 ...
 */
export type KadenaSeedType = "koala" | "chainweaver" | "eckowallet";
 ...
 */
export interface ResolvedKadenaKeypair {
  publicKey: string;
  privateKey: string;
  seedType?: KadenaSeedType | "foreign";
  encryptedSecretKey?: unknown;
  password?: string;
}

/**
 * The injected Kadena crypto/wallet seam. The caller (D5) binds the real
...
export interface HeadlessResolverDeps {
  ...
  /** Binds `KadenaWalletBuilder.createWalletPairFromMnemonic`. `secretKey` is opaque. */
  deriveKadenaKeypair(
    password: string,
    mnemonic: string,
    index: number,
  ...
  collectCodexPubs(kadenaSeeds: KadenaSeedLike[], pureKeypairs: PureKeypairLike[]): Set<string>;
}
