 ...
 *     an existing on-disk entry with no `chainId` reads as the Kadena default
 *     (`entry.chainId ?? KADENA_CHAIN_ID`); the stored entry is UNCHANGED.
 *   - `KADENA_CHAIN_ID` — the default chainId a legacy entry resolves to. It
 ...
 *     (FIX-5): the Kadena validator INTERNALLY dispatches on the orthogonal
 *     `AddressBookEntry.type` (address-KIND), preserving the three per-type
 *     validators verbatim.
 *   - `kadenaAddressValidator` — the concrete Kadena validator T9.8 registers
 *     under `KADENA_CHAIN_ID`.
 *
 ...
 * (CHAIN). The Kadena chain-validator is a thin dispatcher over the three
...
// fails loudly — the additive contract is "read as Kadena", not "rewrite".
...
describe("chainId additive default-to-Kadena (D-10)", () => {
  it("resolves a legacy entry with NO chainId to the Kadena default on read, leaving the stored entry unchanged", () => {
    // KADENA_CHAIN_ID must be a concrete, non-empty chainId string (shared with
    ...
    expect(typeof KADENA_CHAIN_ID).toBe("string");
    expect(KADENA_CHAIN_ID.length).toBeGreaterThan(0);

    const entry = legacyEntry();
    // The read-time default is `entry.chainId ?? KADENA_CHAIN_ID` — a legacy
    // entry has no chainId, so it resolves to Kadena.
    expect(entry.chainId).toBeUndefined();
    expect(entry.chainId ?? KADENA_CHAIN_ID).toBe(KADENA_CHAIN_ID);
    // Additive contract: reading the default must NOT write chainId onto disk.
    expect("chainId" in entry).toBe(false);
  });

  it("lets a NEW entry carry an explicit chainId that overrides the Kadena default", () => {
    ...
    // from the Kadena default (which T9.8 must export as a real constant).
    // The Kadena default must be a real exported string, distinct from the
    ...
    expect(foreign.chainId).not.toBe(KADENA_CHAIN_ID);
    expect(foreign.chainId ?? KADENA_CHAIN_ID).toBe("arweave:mainnet");
  });
  ...
  it("makes chainId a NEW OPTIONAL field — an entry omitting it still satisfies AddressBookEntry (superset, not a changed shape)", () => {
    ...
    // The optional field defaults to Kadena when absent — anchoring the read
    // path on the real KADENA_CHAIN_ID constant T9.8 must export.
    expect(entry.chainId ?? KADENA_CHAIN_ID).toBe(KADENA_CHAIN_ID);
    expect(KADENA_CHAIN_ID).toBeDefined();
  });
});
...
describe("pluggable per-chain validation registry (FIX-5)", () => {
  ...
  it("dispatches validateAddress(KADENA_CHAIN_ID, addr, type) to the registered Kadena validator", () => {
    registerChainAddressValidator(KADENA_CHAIN_ID, kadenaAddressValidator);
    // A well-formed ouronet address under the Kadena chain validates true; the
    // dispatch reached the Kadena validator (not a default-allow).
    expect(validateAddress(KADENA_CHAIN_ID, "Ѻ.recipient", "ouronet")).toBe(true);
    // And a malformed one validates false — proving real validation ran, not a
    // blanket pass-through.
    expect(validateAddress(KADENA_CHAIN_ID, "not-ouronet", "ouronet")).toBe(false);
  });
  ...
  it("registers a SECOND chain's stub validator without disturbing Kadena's, and lists both (E-series slots Arweave in with zero core change)", () => {
    const arweaveLikeStub = (): boolean => true;
    registerChainAddressValidator(KADENA_CHAIN_ID, kadenaAddressValidator);
    registerChainAddressValidator("arweave-like", arweaveLikeStub);

    // Kadena's dispatch is untouched by the second registration.
    expect(validateAddress(KADENA_CHAIN_ID, "Ѻ.recipient", "ouronet")).toBe(true);
    ...
    expect([...getRegisteredChains()].sort()).toEqual(["arweave-like", KADENA_CHAIN_ID].sort());
  });
  ...
  it("returns a FRESH, isolated registry from createAddressValidatorRegistry() — registrations do not leak between instances", () => {
    ...
    registry.register(KADENA_CHAIN_ID, kadenaAddressValidator);
    expect(registry.getRegisteredChains()).toEqual([KADENA_CHAIN_ID]);
...
describe("the 3-type regression under the Kadena default-chain validator (FIX-5 / D-10 non-breaking contract)", () => {
  beforeEach(() => {
    resetAddressValidators();
    registerChainAddressValidator(KADENA_CHAIN_ID, kadenaAddressValidator);
  });
  ...
  it("rejects a malformed OURONET address false (missing the 'Ѻ.' prefix) — per-type strictness is real, not a blanket true", () => {
    expect(validateAddress(KADENA_CHAIN_ID, "recipient-no-sigil", "ouronet")).toBe(false);
  });
  ...
  it("validates a well-formed STOA address true for each k:/c:/w:/u: prefix (/^[kcwu]:/) — the stoa per-type validator is preserved", () => {
    for (const addr of ["k:abc", "c:abc", "w:abc", "u:abc"]) {
      expect(validateAddress(KADENA_CHAIN_ID, addr, "stoa")).toBe(true);
    }
  });

  it("rejects a malformed STOA address false (no k:/c:/w:/u: prefix) — per-type strictness is real, not a blanket true", () => {
    expect(validateAddress(KADENA_CHAIN_ID, "x:abc", "stoa")).toBe(false);
    expect(validateAddress(KADENA_CHAIN_ID, "abc", "stoa")).toBe(false);
  });
  ...
  it("validates a well-formed STOIC-TAG address true (a bare tag, stripSigil(v).length > 0) — the stoic-tag per-type validator is preserved", () => {
    ...
    expect(validateAddress(KADENA_CHAIN_ID, "mytag", "stoic-tag")).toBe(true);
    expect(validateAddress(KADENA_CHAIN_ID, "§mytag", "stoic-tag")).toBe(true);
  });

  it("rejects an empty STOIC-TAG address false (stripSigil yields empty) — per-type strictness is real, not a blanket true", () => {
    expect(validateAddress(KADENA_CHAIN_ID, "§", "stoic-tag")).toBe(false);
    expect(validateAddress(KADENA_CHAIN_ID, "   ", "stoic-tag")).toBe(false);
  });

  it("does NOT collapse the three per-type validators — an address valid under ONE type is rejected under another (the Kadena validator dispatches on type, not a permissive union)", () => {
    ...
    expect(validateAddress(KADENA_CHAIN_ID, ouronetAddr, "ouronet")).toBe(true);
    expect(validateAddress(KADENA_CHAIN_ID, ouronetAddr, "stoa")).toBe(false);

    // And a k:-address is valid stoa but not a valid ouronet address.
    const stoaAddr = "k:abc";
    expect(validateAddress(KADENA_CHAIN_ID, stoaAddr, "stoa")).toBe(true);
    expect(validateAddress(KADENA_CHAIN_ID, stoaAddr, "ouronet")).toBe(false);
  });
});
...
describe("surface stability — AddressBookEntry is a SUPERSET (D-11 overlap)", () => {
  ...
  it("keeps AddressKind the three-member address-KIND union the Kadena validator dispatches on, ORTHOGONAL to chainId", () => {
    // AddressKind is the address-KIND vocabulary the registry value type's
    // second param uses; the Kadena validator must dispatch on exactly these
    ...
    for (const [kind, addr] of perKind) {
      expect(validateAddress(KADENA_CHAIN_ID, addr, kind)).toBe(true);
    }
  });
});
