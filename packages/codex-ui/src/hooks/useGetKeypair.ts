/**
 * useGetKeypair — returns a stable async function (pub) => IStoaChainKeypair.
 *
 * THE function that OuronetUI's wallet-context exposes today as
 * `getStoaChainKeyPairsByPublicKey`. It delegates to the INJECTED
...
export type GetKeypairFn = (publicKey: string) => Promise<IStoaChainKeypair>;

export function useGetKeypair(): GetKeypairFn {
  const seam = useResolverProvider() as CodexResolverSeam | null;
  return useMemo(() => {
    return (publicKey: string): Promise<IStoaChainKeypair> => {
      if (!seam) {
        throw new Error(
          "useGetKeypair: no resolver-provider. Pass a `resolverFactory` to " +
            "<CodexProvider> so the StoaChain-bound hooks can resolve keypairs."
        );
      }
