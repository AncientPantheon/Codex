/**
 * useCodexIdentity (codex-ui) — thin hook over the v0.3.0 codexIdentity slice +
 * the buildRegisterCodexIdentityTx action.
 *
 ...
 * delegation + re-render-on-change, not crypto.
 */
...
import { useCodexIdentity } from "../src/hooks/index.js";
...
describe("useCodexIdentity", () => {
  ...
  it("buildRegisterTx propagates the action's CodexGuardError when identity exists but no active guard", async () => {
    const adapter = await seededAdapter({
      codexIdentity: fullIdentity(),
      pureKeypairs: [
        { ...activeGuard(), id: "not-a-guard", isCodexGuard: false },
      ],
    });
    const { result } = renderHook(() => useCodexIdentity(), {
      wrapper: mkWrapper(adapter),
